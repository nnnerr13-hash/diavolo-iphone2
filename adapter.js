...
/*
 * Gamepad input is read here, at the same point as keyboard input.
 * Direction keys are emitted as one-step pulses so ResetKey() cannot
 * erase the physical held state. Holding a direction repeats naturally.
 */
var gamepad_input_state = {
    active_index: null,
    last_scan: 0,
    held: {},
    pulse: {},
...
function gamepad_find_active() {
    ...
    if (gamepad_input_state.active_index !== null) {
        var current = pads[gamepad_input_state.active_index];
    ...
    for (var i = 0; i < pads.length; ++i) {
        if (pads[i] && pads[i].connected) {
            gamepad_input_state.active_index = i;
            return pads[i];
        }
    }
    gamepad_input_state.active_index = null;
    return null;
}

function gamepad_button_pressed(pad, index) {
    var button = pad && pad.buttons ? pad.buttons[index] : null;
    return !!button && (button.pressed || button.value > 0.5);
}

function gamepad_set_direction(key_id, is_down, now) {
    var was_down = !!gamepad_input_state.held[key_id];

    if (!is_down) {
        gamepad_input_state.held[key_id] = false;
        gamepad_input_state.next_repeat[key_id] = 0;
        return;
    }

    if (!was_down) {
        gamepad_input_state.held[key_id] = true;
        gamepad_input_state.pulse[key_id] = 1;
        gamepad_input_state.next_repeat[key_id] = now + GAMEPAD_FIRST_REPEAT_DELAY;
        return;
    }

    if (now >= (gamepad_input_state.next_repeat[key_id] || 0)) {
        gamepad_input_state.pulse[key_id] = 1;
        gamepad_input_state.next_repeat[key_id] = now + GAMEPAD_REPEAT_INTERVAL;
    }
}

function gamepad_set_action(pad, button_index, key_id) {
    var is_down = gamepad_button_pressed(pad, button_index);
    gamepad_input_state.button_down[key_id] = is_down ? 1 : 0;
}

function gamepad_update() {
    var now = performance.now();
    /* All getkey() calls in one game pass share one scan. */
    if (now - gamepad_input_state.last_scan < 8) {
        return;
    }
    gamepad_input_state.last_scan = now;

    var pad = gamepad_find_active();
    if (!pad) {
        gamepad_input_state.held = {};
        gamepad_input_state.pulse = {};
        gamepad_input_state.next_repeat = {};
        gamepad_input_state.button_down = {};
        return;
    }
    ...
    gamepad_set_direction(37, gamepad_button_pressed(pad, 14) || x < -GAMEPAD_DEAD_ZONE, now);
    gamepad_set_direction(39, gamepad_button_pressed(pad, 15) || x >  GAMEPAD_DEAD_ZONE, now);
    gamepad_set_direction(38, gamepad_button_pressed(pad, 12) || y < -GAMEPAD_DEAD_ZONE, now);
    gamepad_set_direction(40, gamepad_button_pressed(pad, 13) || y >  GAMEPAD_DEAD_ZONE, now);

    gamepad_set_action(pad, 0, 90);  // A / Cross: action
    gamepad_set_action(pad, 1, 88);  // B / Circle: cancel
    gamepad_set_action(pad, 2, 65);  // X / Square: menu
    gamepad_set_action(pad, 3, 83);  // Y / Triangle: shoot
    gamepad_set_action(pad, 4, 67);  // L1: direction modifier
    gamepad_set_action(pad, 5, 32);  // R1: map
    if (gamepad_button_pressed(pad, 8)) gamepad_input_state.button_down[32] = 1;
    if (gamepad_button_pressed(pad, 9)) gamepad_input_state.button_down[65] = 1;
}

function getkey(key_id) {
    gamepad_update();
    if (gamepad_input_state.pulse[key_id]) {
        return 1;
    }
    if (gamepad_input_state.button_down[key_id]) {
        return 1;
    }
...
function ResetKey(key_id) {
    ...
    /* Consume only the virtual pulse. Physical gamepad hold remains. */
    gamepad_input_state.pulse[key_id] = 0;
}
